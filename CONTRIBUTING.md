...
Thanks for looking at this project. It's a small, deliberately narrow-scoped system — a Slack bot
that turns questions into safe, read-only MongoDB queries via Gemini. Keeping it narrow is a
feature, not an oversight, so please read the "What's out of scope" section before proposing
larger changes.
...
- **Vector search / embeddings.** The current MongoDB deployment is self-hosted, not Atlas, so
  `$vectorSearch` isn't available, and no collection currently has unstructured text fields that
